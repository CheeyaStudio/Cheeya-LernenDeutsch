...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.playQuickTrack = function(path) {
    ...
    audioPlayer.play().then(() => {
      isPlayingAudio = true;
      playPauseBtn.innerHTML = `<svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>`;
      audioWave.classList.remove('paused');
      audioTrackSelect.value = path;
    }).catch(err => {
  ...
  function resetAudioState() {
    ...
    playPauseBtn.innerHTML = `<svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>`;
  ...
  audioPlayer.addEventListener('timeupdate', () => {
    if (audioPlayer.duration) {
      ...
      audioTime.textContent = formatTime(cur);
      audioDuration.textContent = formatTime(dur);
